...
function settingsGet(userId: number) {
  const row =
    getDb()
      .prepare(
  ...
  return {
    ...
    defaultFramework: row.defaultFramework,
...
function settingsUpdate(userId: number, data: Record<string, unknown>) {
  const now = nowUnix();
  ...
  if (existing) {
    ...
    for (const [jsKey, sqlKey] of Object.entries(map)) {
      if (data[jsKey] !== undefined && data[jsKey] !== "***") {
        fields.push(`${sqlKey} = ?`);
    ...
    values.push(userId);
    getDb()
      .prepare(`UPDATE user_settings SET ${fields.join(", ")} WHERE userId = ?`)
      .run(...values);
  } else {
    getDb()
      .prepare(
        `INSERT INTO user_settings
         (userId, defaultModel, defaultFramework, defaultLanguage,
          kimiApiKey, openaiApiKey, claudeApiKey, deepseekApiKey, createdAt, updatedAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        userId,
        data.defaultModel ?? "kimi",
        data.defaultFramework ?? "auto",
        data.defaultLanguage ?? "zh",
        data.kimiApiKey || null,
...
function settingsGetApiKey(userId: number, provider: string) {
  ...
  const row = getDb()
    .prepare(`SELECT ${col} FROM user_settings WHERE userId = ?`)
    .get(userId);
