...
export function Onboarding({ onComplete }: Props) {
  ...
  const handleSaveKeys = useCallback(async () => {
    ...
    try {
      // Auto demo login for local-first experience
      try { await demoLogin.mutateAsync(); } catch { /* optional for local-first */ }
      if (Object.keys(payload).length > 0) {
        await updateSettings.mutateAsync(payload);
      }
    } catch { /* user can skip key config */ }
    setSaving(false);
  ...
  }, [keys, step, onComplete, updateSettings, demoLogin]);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: `linear-gradient(135deg, ${current.color}08 0%, #f8fafc 50%, ${current.color}04 100%)` }}
      />
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        />
        <div
          className="absolute w-[400px] h-[400px] rounded-full blur-[100px] transition-all duration-1000 delay-200"
          style={{
      ...
      <div className="relative w-full max-w-[440px] mx-6">
        ...
        <div className="flex justify-center gap-2 mb-8">
          {STEPS.map((_, i) => (
            <button
              ...
              style={{
                width: i === step ? 32 : 8,
                background: i === step ? current.color : i < step ? '#cbd5e1' : '#e2e8f0',
              }}
            />
          ))}
        </div>
        ...
        {/* Main card */}
        <div
          className={`bg-white/70 backdrop-blur-2xl rounded-[28px] shadow-2xl border border-white/20 p-8 transition-all duration-600 ${
        ...
        >
          {step === 0 && (
            <>
              <div
                className="w-[72px] h-[72px] rounded-[22px] flex items-center justify-center mb-6 mx-auto transition-all duration-500"
                style={{
                  background: `linear-gradient(135deg, ${current.color}, ${current.color}dd)`,
                  boxShadow: `0 16px 48px ${current.color}30`,
                }}
              >
                <Icon className="w-9 h-9 text-white" />
              </div>
              <h2 className="text-[28px] font-bold text-slate-900 text-center tracking-tight leading-tight mb-2">
                欢迎使用 TipAi
              </h2>
              <p className="text-[15px] text-slate-500 text-center mb-2">智能提示词工程平台</p>
              <p className="text-sm text-slate-400 text-center leading-relaxed max-w-xs mx-auto">
                从模糊需求到精准提示词，AI 引导式工作流帮你完成每一步。
              </p>
            </>
          )}
          ...
          {step === 1 && (
            <>
              <div className="flex items-center gap-3 mb-6">
                ...
                {filledCount > 0 && (
                  <span className="ml-auto text-xs font-medium px-2.5 py-1 rounded-full" style={{ background: `${current.color}10`, color: current.color }}>
                    {filledCount}/4
                  </span>
                )}
              </div>
              ...
              <div className="space-y-2.5">
                {API_MODELS.map((model) => {
                  ...
                  return (
                    <div
                      key={model.key}
                      className="relative group rounded-2xl transition-all duration-300 border hover:border-opacity-50"
                      style={{
                        background: hasKey ? `${model.color}06` : '#f8fafc',
                        borderColor: hasKey ? `${model.color}20` : '#f1f5f9',
                      }}
                    >
                      <div className="flex items-center gap-3 px-4 py-3">
                        <div
                          ...
                          style={{ background: `${model.color}10` }}
                        >
                        ...
                        <div className="relative flex-1 max-w-[180px]">
                          <Input
                            ...
                            className="h-8 text-xs rounded-xl border-slate-200 focus:border-opacity-50 pr-8"
                            style={{ borderColor: hasKey ? `${model.color}30` : undefined }}
                          />
                          <button
                            onClick={() => toggleShow(model.key)}
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-0.5 rounded-md hover:bg-slate-100 transition-colors"
                          >
              ...
              <p className="text-[11px] text-slate-400 mt-4 text-center flex items-center justify-center gap-1">
                ...
                Key 经 AES-256-GCM 加密后存储在本地 SQLite
              </p>
            </>
          )}
          ...
          {step === 2 && (
            <>
              ...
              <p className="text-[15px] text-slate-500 text-center mb-4">你的数据永远留在你的设备上</p>

              <div className="space-y-3">
                {[
                  { icon: Shield, text: '所有数据存储在本地 SQLite' },
                  { icon: Key, text: 'API Key 经 AES-256-GCM 加密' },
                  { icon: Globe, text: '可选 Ollama 本地模型，完全离线' },
                ].map((item, i) => (
        ...
        <div className="flex items-center justify-between mt-6 px-1">
          <button onClick={onComplete} className="text-[13px] text-slate-400 hover:text-slate-600 transition-colors px-3 py-2">
            以后再说
          </button>
